...
- 多种音频来源：麦克风，或直接捕获**电脑播放的声音**（系统/标签页音频）
- 问 AI：把记录到的内容发给大模型回答，支持 **OpenAI / DeepSeek / Gemini / Groq** 任选，可选「简略 / 详细」
- 网页内配置 Key：右上角 ⚙ 直接填各家 API Key，保存即生效（密钥只存本机、页面仅显示掩码）
...
- 「音频来源」下拉可选 **麦克风** 或 **电脑播放声音**（捕获系统/标签页音频，需在共享弹窗勾选音频）。
- 「转录引擎」下拉可切换 **云端 Deepgram / 本地离线**。
- 右上角 ⚙ 可配置各家 API Key（详见下文「问 AI」与「保密说明」）。
...
## 三·六、问 AI（OpenAI / DeepSeek / Gemini / Groq）

录完一段内容后，点页面上的「🤖 问 AI」把它作为问题发给大模型回答，可选「简略 / 详细」，
并能在「AI 服务商」下拉里切换用哪家（配了哪几家的 Key，就能选哪几家）。

**配置 Key（两种方式任选）：**

- **网页配置（推荐）**：点右上角 ⚙「API 配置」，填入各家 Key 并保存，**立即生效、无需重启**。
- **手动配置**：把对应 Key 写进 `.env`（见 `.env.example`）。

各家 Key 申请地址：

| 服务商 | 免费情况 | 申请地址 | 默认模型 |
| --- | --- | --- | --- |
| OpenAI (ChatGPT) | 付费 | <https://platform.openai.com/api-keys> | `gpt-4o-mini` |
| DeepSeek | 低价 | <https://platform.deepseek.com/api_keys> | `deepseek-chat` |
| Gemini | 免费额度慷慨 | <https://aistudio.google.com/apikey> | `gemini-2.0-flash` |
| Groq | 免费/极快 | <https://console.groq.com/keys> | `llama-3.3-70b-versatile` |

> 想完全免费：**转录**用「本地离线」引擎，**问答**用 Gemini 或 Groq。
>
> 原理：DeepSeek、Gemini、Groq 都提供「OpenAI 兼容」接口，所以四家共用同一套调用逻辑，可随时切换。
> 可用 `{PROVIDER}_MODEL`（如 `GROQ_MODEL`）在 `.env` 覆盖默认模型。

### 保密说明

- 密钥只保存在本机的 `.env` 文件中（已被 `.gitignore` 忽略，不会进 Git、不会上云）。
- 服务默认只监听 `127.0.0.1`（本机），不对外网开放。
- 后端只向前端返回「是否已配置 + 末 4 位掩码」，**绝不回显完整密钥**。
